...
4. **Phase 4: The Voice Server** - full integration and the runtime device-switch feature made concrete end-to-end (done); DTLS/SRTP handshake (`webrtc-rs`) deferred as its own follow-up.
...
## Current Status
Check the PDR (Project Definition Report) for milestone tracking. M1-M4 are complete, M5 is done except DTLS/SRTP (deferred — see below):
...
- The gateway's Axum app lives in the library (`src/gateway.rs`), with `src/bin/gateway.rs` reduced to a thin `main()` — this is what let `tests/ws_chat.rs` spin up the real router in-process. `/ws` handles `Join`/`Chat` via `ecs::systems::text_broadcast`, fanning chat out to every `TextChannel` Device in a room; `ServerMessage::Chat.from` identifies the sending *User*, not the Device, so multi-device fan-out shows a consistent sender. `term_client` is a real CLI (`tokio::select!` between stdin and the WS stream, no `futures-util` split needed on the gateway side). Verified both automatically (`tests/ws_chat.rs`, including a same-user/two-device fan-out case) and manually with two real `term_client` processes exchanging "Hello".
- **The device-switch feature is real**: `ecs::systems::device_switch` moves `AudioSource` between two Device entities of one User; `Command::PacketReceived` now gates forwarding on the sender actually having `AudioSource`, so the switch changes real behavior, not just a marker. `voice_node` links multiple UDP peers as "devices of one user" via matching packet `ssrc` and accepts a `switch <ssrc> <addr>` control line on its own stdin. `ecs::systems::presence` aggregates User-level online status from whether any Device exists. Gateway exposes `PUT /users/{user_id}/audio-device/{device_id}`. Verified via `tests/device_switch.rs` and live: a third listener peer kept receiving audio from whichever Device was switched to be the source, with zero reconnection on the listener's side; also measured ~0.3ms avg forwarding latency over loopback (PDR's Gold Standard is about the forwarding path, not real mic/speaker hardware).
- **Deferred**: DTLS/SRTP via `webrtc-rs` was scoped for M5 but pulled out into its own follow-up — it's the single largest, most open-ended remaining piece of work, and neither Gold Standard DoD check requires it. Voice packets are forwarded unencrypted for now.

`cargo clippy --all-targets` (with and without `--features audio`) is clean.
